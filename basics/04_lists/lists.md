...
- **List**: is a collection which is ordered and changeable(modifiable). Allows duplicate members.(~ to array(vector) but we can do more things)
  (una lista siempre tiene x debajo un array , pero en python no existe un array con lo cual () y [] es lo mismo)
...
take the items from the list and assigning them to separate variables
